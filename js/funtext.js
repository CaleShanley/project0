    ...
    var methods = {
      ...
      buildShadow: function (ratioX, ratioY) {
        ...
        while (counter++ < base.shadowSize) {
          ...
          shadowRule +=
            ' ' +
            parseInt(counter * ratioX) +
            'px ' +
            parseInt(counter * ratioY) +
            'px 0 ' +
            (switches.solidColor
              ? base.shadowColor
              : base.shadowColor[
                  parseInt(
                    (base.shadowColor.length / base.shadowSize) * (counter - 1)
                  )
                ])
        }
      ...
      watchMouse: function (e) {
        ...
        base.$el.css(
          'text-shadow',
          methods.buildShadow(
            ((e.pageX - window.innerWidth / 2) / (window.innerWidth / 2)) * -1,
            ((e.pageY - window.innerHeight / 2) / (window.innerWidth / 2)) * -1
          )
        )
      },
      ...
      init: function () {
        if (typeof shadowSize === 'undefined' || shadowSize === null) {
          ...
          typeof shadowSize !== 'number' &&
          (typeof shadowColor === 'undefined' || shadowColor === null)
        ) {
        ...
        if (typeof shadowColor === 'undefined' || shadowColor === null) {
          ...
          typeof shadowColor === 'object' ||
          Object.prototype.toString.call(shadowColor) === '[object Array]'
        ) {
        ...
        $(document).on(
          ...
          methods.throttle(methods.watchMouse, 50)
        )
      }
    }
